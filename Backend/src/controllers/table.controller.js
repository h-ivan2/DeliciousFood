...
exports.createTable = async (req, res, next) => {
  try {
    ...
    if (restaurant.owner.toString() !== req.user._id.toString()) {
      return res.status(403).json({ success: false, message: 'Not authorised to add tables to this restaurant' });
    }
...
exports.getTables = async (req, res, next) => {
  try {
    ...
    const tables = await Table.find({ restaurant: req.params.restaurantId }).sort('tableNumber');
...
exports.getAvailableTables = async (req, res, next) => {
  try {
    ...
    if (!date || !time || !partySize) {
      return res.status(400).json({ success: false, message: 'Please provide date, time, and partySize' });
    }
    ...
    const bookedTableIds = overlappingReservations.map(res => res.table.toString());
